test_that("dcm_specification class errors when expected", {
  ...
  expect_error(dcm_specify(qmatrix = test_qmatrix),
...
test_that("dcm_specification works", {
  ...
  expect_identical(names(spec@qmatrix_meta), c("attribute_names",
                                               "item_identifier",
                                               "item_names"))
  expect_identical(spec@qmatrix_meta$attribute_names,
                   rlang::set_names(paste0("att", 1:3), paste0("node", 1:3)))
  expect_identical(spec@qmatrix_meta$item_identifier, "item")
  expect_identical(spec@qmatrix_meta$item_names,
  ...
  expect_identical(spec@priors, default_dcm_priors(lcdm(max_interaction = 1),
                                                   unconstrained()))
})
...
test_that("printing works", {
  ...
  spec3 <- dcm_specify(qmatrix = test_qmatrix, identifier = "item",
                       measurement_model = lcdm(max_interaction = 1),
                       structural_model = hdcm("node3 -> node2 -> node1"))

  spec4 <- dcm_specify(qmatrix = test_qmatrix2, identifier = "question",
                       measurement_model = dina(),
